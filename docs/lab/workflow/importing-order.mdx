...
Since the request represents an existing order, we don’t perform strict address validation on the patient address, to make it easier to import orders exactly as they were placed in the different interface originally.
...
The response object schema is the same as [creating an order](/api-reference/lab_testing/create-order).
