...
7. Use the returned resource as the source of truth for the final state.
...
| `sub_reason_codes` | Additional concerns, such as a lab-account mismatch. |
...
| `matched` | The result has been matched. This state is not available as a list filter. |
...
| --- | --- | --- |
| `match_sample_id` | The sample identifier matched an existing order. This normally matches automatically, but an additional concern can require review. | Confirm the sample, patient, and any `sub_reason_codes`. |
...
### Lab-account mismatch

`lab_account_mismatch` is a `sub_reason_code`, not a separate decision case. It
means the provider account identifier in the incoming result does not match the
lab account associated with the candidate order. Treat it as an additional
identity and routing check before accepting the result.
...
| Account and lab | `lab_account_mismatch`, `possible_wrong_lab`, `same_lab` |
...
- Store the order returned by `accept`.
