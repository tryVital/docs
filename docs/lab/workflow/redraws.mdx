...
- Order transactions have a `status` field, which can either be `active`, `completed` or `cancelled`
  ...
  - `cancelled` - the initial order has been cancelled (and there's no redraw order), or the redraw order has been cancelled
...
We recommend that you store both the `order_id` and `order_transaction_id` when placing an order in Junction. This way, if you receive a webhook for an order that you have not seen in your system, you will be able to link it to its parent via the `order_transaction_id`. 
