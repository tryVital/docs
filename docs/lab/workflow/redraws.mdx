...
We recommend that you store both the `order_id` and `transaction_id` when placing an order in Junction. This way, if you receive a webhook for an order that you have not seen in your system, you will be able to reference it to it's parent via the `transaction_id`. From there, you can always use the `transaction_id` directly to obtain results, ensuring you always get the complete set of results for a particular order transaction.