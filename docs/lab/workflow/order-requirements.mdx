...
For placing insurance orders with Spiriplex, you need the following:
...
You can provide this information with our API, using the `physician` and `health_insurance` arguments for the [POST v3/order/](/api-reference/lab-testing/create-order) REST endpoint.
...
After you place an order with all the required information, a requisition form is generated and sent to Spiriplex
with all the necessary information. Your customer’s Health Insurance can then be billed by Spiriplex.

#### Labcorp

To place an Order with Health Insurance as the billing method in Labcorp, you can use the `health_insurance` argument 
for the [POST v3/order/](/api-reference/lab-testing/create-order) REST endpoint, by providing:
