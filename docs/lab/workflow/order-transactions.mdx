...
Store both `order.id` and `order_transaction.id` from the Create Order response. Use the order ID for an individual order and the transaction ID for workflows involving related orders, including [redraws](/lab/workflow/redraws).
