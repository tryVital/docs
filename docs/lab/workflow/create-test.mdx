...
description: "Create custom lab tests by selecting biomarker IDs and collection methods, then submit them for Junction approval before ordering."
---
...
Note that `status` is set to `pending_approval`. You can verify when it changes to `active` by calling the [lab tests endpoint](/api-reference/lab-testing/tests). The `is_active` field is deprecated; use `status` instead.
