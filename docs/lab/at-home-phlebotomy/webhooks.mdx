...
There are some webhook events that are of interest when placing an At-Home Phlebotomy order. Those are
...
<Info>
    The `labtest.appointment.updated` event will be triggered for all possible appointment statuses, and is the **recommended** way of integrating to fetch all
...
<Info>The `event_data` field contains pertinent information regarding the current appointment status, and may be specific for each `provider`.</Info>