...
<Steps>
  ...
  For an [Appointment Ready Tier](/lab/at-home-phlebotomy/service-tiers) service, the Appointment starts with the `reserved` status.
  ...
  1. The Order would move through the `requisition_created` and `appointment_pending` Order statuses in quick succession.

  You would receive two `labtest.order.updated` events, one each for the two status transitions.
  ...
  1. The Order would move further to the `appointment_scheduled` Order status in quick succession.
  2. The Appointment would move to the `confirmed` status automatically, which corresponds to the `scheduled` appointment event.

  You would additionally receive a `labtest.order.updated` event and a `labtest.appointment.updated` event for the status transition.
  </Step>
</Steps>
...
* The Appointment would move to the `cancelled` status; and
* There is no change to the Order, which would remain in the `ordered` status.

If an Appointment is cancelled **after** a requisition has been created:

1. The Appointment would move to the `cancelled` status; and
2. The Order would move from the `appointment_pending` or `appointment_scheduled` status to the `appointment_cancelled` status.
