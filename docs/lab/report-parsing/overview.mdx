...
2. Junction queues the job and extracts report metadata and lab results.
3. Junction attempts to match extracted results to LOINC codes.
...
sequenceDiagram
    ...
    Parser->>API: Upload file, extract results, and match LOINCs
...
| Multiple files | JPEG and PNG only | Up to 8 files, 20 MiB combined |
| PDF pages | PDF upload or merged image upload | Up to 20 pages |
...
| `failed` | Parsing failed. Check `failure_reason` for the machine-readable reason. |

`failure_reason` can include:

| Failure reason | Meaning |
| --- | --- |
| `invalid_input` | The uploaded file is not a supported or parseable report input. |
| `low_quality` | The document quality was too low to reliably extract results. |
| `not_english` | The report language is not supported. |
| `too_many_pages` | The report exceeded the parser page limit. |
...
Set `needs_human_review` to `true` when you want Junction to manually review extracted results before the job is finalized. Human review is useful for high-impact workflows, low-quality scans, complex multi-page reports, or reports where your application needs a higher-confidence extraction path.
...
| Field | Meaning |
| --- | --- |
| `needs_human_review` | Whether the job was submitted for manual review. |
| `is_reviewed` | Whether manual review has been completed. |
...
| `gender` | Extracted patient gender when present. |
...
| Field | Description |
| --- | --- |
| `test_name` | Marker or observation name as extracted from the report. |
...
| `interpretation` | Parsed interpretation: `normal`, `abnormal`, `critical`, or `unknown`. |
...
| `percentage` | `"5.4"`, `"5.4%"` | Percentage results may also include `%` in `units`. Handle both forms. |
| `ratio` | `"97/100"`, `"1:80"` | Ratio formats vary by report. Slash-form ratios are inferred when `type` is not already supplied. |
...
- `interpretation`: `normal`, `abnormal`, `critical`, or `unknown`
