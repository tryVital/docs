...
```python Python
job = client.lab_report.parser_get_job("<job_id>")
...
```python
def handle_patient_upload(file, user_id):
    job = client.lab_report.parser_create_job(
        file=[file],
        user_id=user_id,
    )

    # Store job_id linked to user for later retrieval
    store_pending_job(user_id, job.id)
