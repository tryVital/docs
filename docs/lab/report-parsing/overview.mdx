...
<Info>
LOINC matching enables you to compare results across different labs and integrate with other health systems using standardized identifiers.
</Info>
...
def handle_patient_upload(file, patient_id):
    ...
    store_pending_job(patient_id, job.job_id)
...
for filename in os.listdir("historical_reports/"):
    with open(f"historical_reports/{filename}", "rb") as f:
        ...
        print(f"Created job {job.job_id} for {filename}")
```
