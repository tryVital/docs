...
| `failed` | Parsing failed. Check `failure_reason` when present for the machine-readable reason. Some internal processing errors may return `failure_reason: null`. If this happens, retry the job or contact support with the job ID. |
