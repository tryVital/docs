...
description: "Reference for all order and appointment lifecycle statuses in the walk-in flow, from order placement through appointment and results."
---
...
For each modality, there can be multiple **low-level statuses**, for **Walk-In Test** the
...
- `redraw_available`: The result has been marked as final but is missing biomarker results due to a lab error. (Currently in beta testing.)
...
<Frame caption="Possible state transitions for the Walk-In Test Order low-level statuses.">
...
- `pending`: An appointment was placed in the system, and is pending updates from the scheduling service.
...
<Frame caption="Possible state transitions for the Walk-In Test Appointment statuses.">
...
and returned when using the [GET Appointment endpoint](/api-reference/lab-testing/psc-scheduling/get-psc-appointment).
...
<Steps>
  ...
  You will receive three `labtest.order.updated` events, one each for the three Order status transitions. You will also receive
...
<Steps>
  ...
  You will receive two `labtest.order.updated` events, one each for the two Order status transitions. You will also receive
  a `labtest.appointment.updated` event for the Appointment status transition.
  </Step>
</Steps>
