...
The events are related to a single appointment. An order can have multiple existing appointments in Junction system, although only one appointment will be considered active
...
<Steps>
  ...
  You will receive two `labtest.order.updated` events, one each for the two Order status transitions. You will also received
  a `labtest.appointment.updated` event for the Appointment status transition.
  </Step>
</Steps>
