...
With Async Confirmation enabled, Junction waits synchronously for the provider to acknowledge the booking request, up to the sync confirmation timeout (`sync_confirmation_timeout_millisecond`).
...
If a pending appointment is created, Junction continues to retry the booking request with the PSC Appointment Provider in the background, up to the async confirmation timeout (`async_confirmation_timeout_millisecond`).
