...
3. When an order reaches `completed`, call [Get Order Results](/api-reference/lab-testing/results/get-results).
4. Store or display the structured result according to its `status`, `interpretation`, and result types.
5. Retrieve the [result PDF](/api-reference/lab-testing/results/get-results-pdf) when your workflow needs the laboratory report and one is available.
...
For supported walk-in orders, a laboratory issue can produce a `redraw_available` status and a related redraw order. Use `order_transaction.id` to associate the initial and redraw orders and retrieve the complete result set. See [Redraws](/lab/workflow/redraws) and [Order Transactions](/lab/workflow/order-transactions).
