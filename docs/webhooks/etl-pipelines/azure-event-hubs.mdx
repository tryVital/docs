...
description: "Stream Junction events to Azure Event Hubs as an ETL pipeline destination, with details on event structure, properties, and compression."
icon: "microsoft"
---
...
<Info>
At this time, Junction publishes exclusively JSON blobs (`blob_type == json`).
...
<Warning>
Some polling-based providers can write 96 writes per day per user on certain event types, e.g., `daily.data.activity.*` and
