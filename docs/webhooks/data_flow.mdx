...
When data is available, the backend triggers a `historical.data.<resource_type>.created` webhook event. The `resource_type` is determined by the `provider` - more information on which resources a provider can support can be found [here](https://vital-docs.readme.io/docs/resources).
