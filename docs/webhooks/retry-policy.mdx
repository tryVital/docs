---
title: "Retry Policy"
description: "Understand the Junction webhook retry schedule with exponential backoff across eight attempts, from immediate retry up to 10 hours between attempts."
---
...
If all attempts to a specific endpoint fail for a period of 5 days, the endpoint will be disabled. The clock only starts after multiple deliveries failed within a 24-hour span, with at least a 12-hour difference between the first and the last failure.
