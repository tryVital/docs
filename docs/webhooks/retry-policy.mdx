...
The way to indicate that a webhook has been processed is by returning a `2xx` (status code `200-299`) response to the webhook message within a reasonable time-frame (15s). Any other status code, including `3xx` redirects, is treated as a failure.
