...
- Verify webhook signatures as described in [Authenticating Webhooks](/webhooks/introduction#authenticating-webhooks).
- Return a `2xx` response within 15 seconds to acknowledge delivery.
- Make processing idempotent because unsuccessful deliveries are retried according to the [webhook retry policy](/webhooks/retry-policy).
- Monitor failed deliveries and disabled endpoints in the Junction Dashboard.
- The user, provider, `error_type`, `svix-id`, and receipt time can help diagnose delivery or connection issues.

With this lifecycle in place, connection failures can become a self-service reconnection flow.
