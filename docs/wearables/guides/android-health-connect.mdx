...
description: "Integrate Android Health Connect as an SDK-based provider with Junction, including setup, permissions, and required Mobile SDK components."
---
...
[Junction Health SDK](/wearables/sdks/vital-core) into your Native iOS, React Native or Flutter mobile app.
...
Follow the [Junction Health SDK](/wearables/sdks/vital-health) starting guidance to
...
Junction Health SDK runs all its data sync workers inside a short-service Foreground Services.
...
The augmentation above is a workaround to React Native internals AndroidX-incompatible handling of permission request result callbacks,
...
experience with Android Health Connect, similar to [Background Delivery for Apple HealthKit](/wearables/guides/apple-healthkit#background-delivery) on iOS.
