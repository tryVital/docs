...
experience with Android Health Connect, siimlar to [Background Delivery for Apple HealthKit](/wearables/guides/apple_health_kit#background-delivery) on iOS.


### Sync Frequency

The SDK schedules hourly background sync using the Android [Exact Alarms](https://developer.android.com/develop/background-work/services/alarms/schedule) mechanism.
...
<Tabs>
    ...
    an Android Intent to request the _"Alarms & Reminders"_ permission. Otherwise, the contract returns `true` synchronously.
    ...
    Otherwise, `enableBackgroundSync()` returns `true` immediately.

    You can also inspect if Background Sync has been enabled through the `isBackgroundSyncEnabled` property.

    <Note>
    When running on iOS, both APIs return `true` immediately.
    </Note>
    ...
    ```typescript Android Compose
    import { VitalHealth } from "@tryvital/vital-health-react-native";

    await VitalHealth.enableBackgroundSync();
    ```
    </Tab>
</Tabs>
