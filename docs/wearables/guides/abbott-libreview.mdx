...
Vital has launched a new **Abbott LibreView integration** (`abbott_libreview`) for Abbott CGMs:

* Users can connect their CGM devices simply by signing in with their LibreView patient account.
* You no longer need to maintain a LibreView practice.
* You no longer need to instruct your users to share/connect to your practice, before they attempt to connect via Vital Link. 

The practice-based `freestyle_libre` integration will remain active to serve existing connections.

| Provider Slug      | Remarks                   |
| ------------------ | ------------------------- | 
| `abbott_libreview` | Connect via LibreView patient account | 
| `freestyle_libre`  | _Deprecated_              |
...
## Connecting a LibreView patient account
...
## Setting up practice connections (Legacy integration)

<Warning>
This guide applies **only** to the legacy `freestyle_libre` integration that relies on practice-based connections.
</Warning>
