...
<Note>
Strava is BYOO-only. Junction's default Strava OAuth application is unavailable for new Strava connections and reconnects. Google Health API is BYOO-only until Junction's default application is approved.
</Note>
