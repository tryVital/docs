...
<Note>
Vital does not guarantee a stable JWKS location nor a stable issuer on these JWT tokens.

Verifying this specific JWT token is unnecessary, since the claim payload format does not open
up any usable CSRF attack vector.
</Note>
...
| Key                     | Value                                           |
| ----------------------- | ----------------------------------------------- |
| `vital_link`            | Constant `true`                                 |
| `team_id`               | Vital Team ID                                   |
| `user_id`               | Vital User ID                                   |
| `token_id`              | Vital Link Token ID                             |
| `env`                   | Vital Environment                               |
| `region`                | Vital Region                                    |
</Accordion>
...
def handle_oauth_callback(
    ...
    if (
        ...
        and "vital_link" in unverified_claims
    ):
