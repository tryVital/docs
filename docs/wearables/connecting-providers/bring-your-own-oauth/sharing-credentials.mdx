...
with a specific format. This `state` query parameter is typically passed through with no change, when
the provider performs the final redirect to the Redirect URI.

Your OAuth callback endpoints can rely on the passthrough `state` parameter to differentiate the request
origin — i.e., whether it is from Vital or from your own production systems.

When you detect the request is originated from Vital, please perform a 307 Temporary
Redirect to the [Vital OAuth callback endpoint](#vital-oauth-callback-endpoint-url) and passing on _all_ the URL query parameters.
...
<Warning>
While you need not verify the signature of this JWT, to prevent Cross-Site Request Forgery attacks,
you must check that the `aud` claim matches the expected [Vital Link API Base URL](#vital-link-api-base-url).
</Warning>
...
def handle_oauth_callback(
    ...
    if (
        ...
        return fastapi.RedirectResponse(f"{VITAL_LINK_BASE_URL}/link/fitbit?{query}")
