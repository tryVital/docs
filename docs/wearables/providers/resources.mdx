...
Providers mark with `*` means that Junction calculates the field's value. In some cases, they might came as `null`, since we weren't able to make the calculation.
