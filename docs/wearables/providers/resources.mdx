---
title: "Resources"
description: "Understand the two wearable data categories in Junction: Summaries for daily aggregates and Timeseries for granular time-stamped measurements."
---
...
`Heartrate` on the other hand, is a timeseries resource. You can fetch it via `timeseries/<user_id>/heartrate` and looks like this:
...
Resources alone are not the whole story. Depending on the provider, a field might not be available. For example both Garmin and Strava provide `Workouts`, yet the former doesn't have `elev_high` nor `elev_low`.

<Note>

Providers marked with `*` means that Junction calculates the field's value. In some cases, they might come as `null`, since we weren't able to make the calculation.
