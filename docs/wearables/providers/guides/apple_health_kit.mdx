...
<Tip>
  Independently of the plarform you are using, you should read the [Native iOS
  Guide](/wearables/sdks/ios), since the docs apply to all platforms.
</Tip>
...
From our experience with how the SDK is used, what tends to happen is a flow that allows the end-user to connect with Apple HealthKit. So a screen would ask the user if they would like to sync their Apple HealthKit. If the user accepts, you would then call:

1. `VitalClient.configure`
2. `VitalClient.setUserId`
3. `VitalHealthKitClient.configure`

These three calls alongside `VitalHealthKitClient.automaticConfiguration` at `didFinishLaunchingWithOptions` tends to be enough to remove most issues we have seen when integrating with Apple HealthKit.
