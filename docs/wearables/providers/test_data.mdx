...
where the number of days data backfilled varies according to the provider. [Historical backfill] (wearables/providers/introduction#historical_days_retrieved)

## Data Format

When connecting to the live Vital client, there are three data response structures available for data. 
Raw, Stream and Vital

When connecting to the demo Vital client data is only available in the Vital structure.

Vital restructures provider data fields, to have a consistent response structure across all providers. 
