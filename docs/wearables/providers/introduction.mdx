...
Whenever Junction detects that new data is available, [data events](/event-catalog) are always sent to your configured endpoints, regardless of the provider being push-based or polling-based.
...
- Push-based
  ...
  - When a cloud-based provider supports a push notify mechanism (typically webhooks), Junction would prefer to use it to drive data fetches. In other words, as soon as the provider notifies Junction of new data through the said mechanism, Junction fetches the latest data.

- Polling-based

  - For providers and/or resources without any push notify mechanism, Junction polls these resources at a regular schedule, typically every 15 minutes or so.
...
<Info>
The Team Data Pull Preferences you specified is _advisory_. There are scenarios in which Junction systems
