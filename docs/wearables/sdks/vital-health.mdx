...
If your app has already did, any further attempt to [Ask for Permission](#ask-user-for-health-data-permissions) would
be gracefully ignored by the operating system.

The only exception to this behaviour is when a `VitalResource` pulls a new data type introduced in a new OS release.
In this scenario, calling [Ask for Permission](#ask-user-for-health-data-permissions) would result in a permission
prompt that asks for permissions to read these new data types.
...
When designing your user experience, **do not assume** you can re-engage the end user after the
[Ask for Permission](#ask-user-for-health-data-permissions) based on permission grants and denials.
