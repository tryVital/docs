...
<Steps>
  ...
  <Step title="Your backend <-> Vital API">
    ...
    You then send back the response back to your app's user:
  ...
  <Step title="Your backend -> Your app">
    Your app receives the API response and the Vital Sign-In Token. It then signs in with the Vital Mobile SDK using the `sign_in_token`.
