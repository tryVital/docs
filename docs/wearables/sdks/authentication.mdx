...
2. When your user has requested to sign out from your app, it instrucs the Vital Mobile SDK to reset its state.

Your mobile apps typically only need to request the Vital Sign-In Token once through your backend service, and use the obtained Vital's `sign_in_token` with the Vital Mobile SDK. Once signed-in, the Vital Mobile SDK maintains its user session in persistence storage.
