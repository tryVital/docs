...
* The SDK automatically signed out, because it had detected the deletion of the Vital User (through refresh token flow failure).
...
2. The Vital SDK sign-in state could be out of sync with your app's user lifecycle, depending on how you
   architect the integration; and
