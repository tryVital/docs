...
a **Vital Sign-In Token** for the user, through your own API authentication and authorization system.
...
You would sign-in and sign-out of the Vital Mobile SDK as an integral part of your app user lifecycle:
...
Generally speaking, you sign in a user only once with the Vital Mobile SDK. This is because Vital Mobile SDK
persists the signed-in user — as well as any settings and sync state — across app relaunches and device reboot. 

Having said that, you might want to reconcile the Vital SDK sign-in state with your app's own user state as well.
For example, you can do this during app launch. You can do this through the [Core SDK](/wearables/sdks/vital-core#core-sdk-status) inspection methods.

If you are integrating Vital into your existing production app, such reconcilation process would also act as a one-off migration to
setup the Vital SDK for your users upgrading from an older app release.

<Info>
It is unnecessary to request and sign-in Vital Sign-In Token every time your app launches.
You only need to sign-in a user once.
</Info>

<Warning>
Do not sign in and sign out every time your app relaunches.
</Warning>
...
<Steps>
  ...
  <Step title="Your backend service <-> Vital API">
    ...
    Your backend API receives and validates the token issuance request. Once validated, your backend service uses the
    Vital API Key — kept as a server-side secret — to fuilfill the request:
