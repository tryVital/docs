  ...
  <Step title="Your backend API -> Your app">
    ...
    <CodeGroup>
      ...
      try {
        ...
        Fimble.i("Signed in with Junction successfully")
      } catch (error) {
        Fimble.e("Error signing in with Junction", error)
      }
      ```
    </CodeGroup>
    <br />
...
Use [Junction Sign-In Token](#vital-sign-in-token) whenever possible.
</Warning>
...
Alternatively, follow the [Apple HealthKit guide](/wearables/guides/apple-healthkit#2-integrate-with-ios-system-callbacks)
...
<Warning>
Always use [Junction Sign-In Token](#vital-sign-in-tokens) for your production mobile apps.
</Warning>
