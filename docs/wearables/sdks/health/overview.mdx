...
2. Configured Android Background Sync for the provider you are using — [Health Connect](/wearables/guides/android-health-connect#background-sync)
   or [Samsung Health](/wearables/guides/samsung-health#background-sync) — and request user permissions (when necessary) as per instruction.
...
are welcomed to dig into it for a self-troubleshooting attempt.
</Tip>
