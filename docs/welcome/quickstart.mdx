...
Start by creating an account at https://app.tryvital.io. After registered, you can create a team. A team is associated with a region (either EU, or US), please select the one most suitable for you. Once done, you will have access to API Keys for both Sandbox and Production.
...
You create a _user_ with the `/user/` endpoint. Once this is done, you will get an `user_id`. Most of our API requests require an `user_id` as input. When creating a user, you need to pass an unique id. This represents the user in your system.
