...
We havei improved the search and filtering capabilities in the Orders page, helping you quickly locate specific orders and information.
...
Filter specifically for tests that are scheduled in the future (Activation Type = Scheduled) or test kits that were either not scheduled, or were scheduled in some date in the past (Activation type = Current).
