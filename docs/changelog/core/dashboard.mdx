...
### Native Order Set Building (Mar 2026)

You can now build custom order sets during order creation by combining pre-defined panels with individual à-la-carte markers, and optionally removing markers from selected panels.

<Accordion title="Details" defaultOpen icon="flask-vial" iconType="duotone">
  ## What's New

  - **Mixed panel and marker selection** – Add whole panels and individual à-la-carte markers to the same order.
  - **Inline marker removal** – Remove specific markers from a selected panel without affecting the original panel definition.
  - **Order set review** – The review screen shows a full breakdown of panels, modified panels, and à-la-carte markers.
  - **Save as team panel** – When your order set differs from existing panels and all markers belong to the same lab, you can save it as a reusable team panel with a name, description, collection method, and fasting requirement.
</Accordion>
