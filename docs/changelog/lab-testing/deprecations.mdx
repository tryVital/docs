...
We are removing the `value` field from the [results object](/api-reference/results/get-results), as it does not accurately capture all possible result values. In turn, we've introduced the `result` field.
