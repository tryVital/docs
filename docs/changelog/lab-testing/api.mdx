...
### More specific error when marking on-site collection order as draw completed (May 2026)

On-site collection orders cannot be marked as draw completed with the [draw completed](/api-reference/lab-testing/update-order-draw-completed) endpoint if the order is in a terminal state (e.g., the order is already completed). Previously, it wasn't clear that the state was the reason for failure. The new error message explicitly tells you that the order is in a terminal state and includes the state.
