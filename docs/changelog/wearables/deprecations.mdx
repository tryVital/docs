...
We have introduced [Get Grouped Timeseries](/changelog/wearables/api#grouped-timeseries-feb-2024) endpoints since Feburary 2024.
Checkout the [Get Heartrate](/api-reference/data/timeseries/heartrate) endpoint for an example. It has been supported by all Junction API
...
1. It uses cursor-based pagination, allowing you to fetch and process data incrementally.
2. It exposes the [data source attributions](/wearables/providers/data-attributions), which is absent from the legacy endpoints.
...
For direct API calls, make sure your request URL paths do include `/grouped`, your logic do expect the grouped timeseries response schema, and that you handle pagination.
</Card>
