...
### Continuous Query sleep macros (September 2025)

Continuous Query now exposes two new Sleep Macros to help you pinpoint when users fall asleep and wake up within a session.

<Accordion icon="moon" iconType="duotone" title="Details" defaultOpen>

- `asleep_at` returns the sleep session's bedtime start adjusted by the recorded sleep latency.
- `awake_at` returns the bedtime start plus the offset of the final non-awake sleep segment, helping you identify the exact moment of the user waking up.
