...
Junction Sense queries now support a new `.align()` clause that fills empty time buckets in your aggregated output with values carried over from neighbouring buckets. Use it when you want a contiguous time series from sparse input - for example a daily weight composite from a once-a-week weigh-in.
...
va.select(va.group_key("*"), va.Body.col("weight_kilogram").mean())
  .group_by(va.date_trunc(va.Body.index(), 1, "day"))
...
For the full reference including operator semantics, validation rules, and per-partition behaviour, see the [ALIGN clause documentation](/sense/query-dsl/align-clause).
