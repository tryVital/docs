...
We have added the ability to **group the input rows by any attribute** in the Horizon AI Query DSL, including both summary and timeseries resources.
This gives you more flexibility to analyze your health data across different dimensions — including source providers, data types, and custom attributes in the datasets.

Check out the [Group BY](/api-reference/horizon-ai/aggregation/query-dsl/group-by-clause) documentation.
...
# Group Electrocardiogram Data by Hour and Lead Type

va.select(...).group_by(
   va.date_trunc(va.Timeseries.index(), 1, "hour"),
   va.Timeseries.col("electorcardiogram_voltage").field("type"),
)
...

{
    "group_by": [
        ...
        {
            "timeseries": "electorcardiogram_voltage",
            "field": "type"
        },
    ]
}
...
With a few simple configuration steps, Continuous Query enables you to extract aggregations of high‑volume timeseries data collected across all your active user device connections. 
Moreover, offloading this concern to Junction would help simplify your data pipeline design, allowing your team to focus on extracting actionable signals for your end users and use cases.
