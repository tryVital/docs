...
### Horizon AI Query SleepSnapshot scores (March 2026)

Horizon AI Query now supports `sleep_snapshot`, a table that returns daily sleep-derived recovery and strain insights out of the box.

Use it to query `sleep_score`, `recovery_score`, `stress_score`, `strain_score`, their zones, and `chronotype` without building custom scoring logic.

Check out the [SleepSnapshot Scores](/api-reference/horizon-ai/aggregation/query-dsl/sleep-snapshot-scores) documentation for full details.

<Accordion icon="bed" iconType="duotone" title="Details" defaultOpen>

- New query table: `sleep_snapshot`
- New index expression: `{ "index": "sleep_snapshot" }` / `SleepSnapshot.index()`
...
- Check out the [WHERE clause](/api-reference/horizon-ai/aggregation/query-dsl/where-clause) documentation for more details.
...
Horizon AI Query now exposes two new Sleep Macros to help you pinpoint when users fall asleep and wake up within a session.
Check out [Sleep Analysis](/api-reference/horizon-ai/aggregation/query-dsl/sleep-analysis) for more details.
...
Use these macros in the Select clause of Horizon AI queries to surface consistent bedtime and wake-up timestamps across providers.
