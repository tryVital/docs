...
Data payloads now include source_device_id when the originating device can be determined, enabling better tracking and device-specific analytics.
...
Data payloads now include `source_device_id` whenever the originating device is known with certainty. This currently applies to:

- **Withings**: Device metadata embedded in payloads
- **Polar**: Device metadata embedded in payloads
- **Single-device connections**: For all other providers where exactly one device is registered

This enables better data source tracking and device-specific analytics for your applications.
</Accordion>
