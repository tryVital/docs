...
We've expanded Continuous Query to support **timeseries resources** in addition to the previously available summary types.
This makes it easier for you to handle high‑volume timeseries data and eliminates the need to build custom scalable solutions on your backend.

## What's New

- **Query multiple fields** from different timeseries resources in a single operation
- Retrieve both **discrete samples** (e.g. heart rate) and **interval samples** (e.g. workout distance)

<Accordion icon="link" iconType="duotone" title="DSL Example" defaultOpen>
...
select(
    ...
    # Interval Samples
    Timeseries.col("daylight_exposure").field("value"),
    Timeseries.col("daylight_exposure").field("duration"),
    Timeseries.col("daylight_exposure").field("timezone_offset"),
