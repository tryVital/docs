...
Horizon AI Query and Continuous Query now accept a `where` clause so you can filter input rows before aggregation.
...
- DSL helpers expose `.where("…")` so you can keep filtering logic alongside your select and aggregation expressions.
- Read more in the [WHERE clause documentation](/api-reference/horizon-ai/aggregation/query-dsl/where-clause).
