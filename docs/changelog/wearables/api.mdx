...
### Junction Sense Menstrual Cycle resource & list-column aggregation (April 2026)

Junction Sense now supports the `menstrual_cycle` query table, and introduces a subquery syntax for aggregating list-column elements — for example, averaging the basal body temperature entries within each cycle.

Check out [List-column aggregation](/sense/query-dsl/list-column-aggregation) and the updated [Column expressions](/sense/query-dsl/column-expressions#menstrualcycle) reference for full details.
...
- Inside an UNNEST `where` over a scalar-element list (e.g. `List[Utf8]`), the reserved identifier `element` refers to the element value itself.
