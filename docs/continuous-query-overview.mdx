...
Most resources use the same Query DSL with both products, making it easy to prototype with Query API and deploy to production with Continuous Query. Some virtual resources are only available through Continuous Query [Device Reliability](/sense/query-dsl/device-reliability) must be validated and created as a Continuous Query rather than run through Query API.
