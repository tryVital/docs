...
Use the Junction Dashboard Query Editor to preview resources supported by Query API. For a Continuous Query-only resource such as Device Reliability, the [Create Continuous Query endpoint](/api-reference/sense/continuous-query/create) validates the query when you submit it.
...
Most resources use the same Query DSL with both products, making it easy to prototype with Query API and deploy to production with Continuous Query. Some virtual resources are Continuous Query-only: [Device Reliability](/sense/query-dsl/device-reliability) must be validated and created as a Continuous Query rather than run through Query API.
