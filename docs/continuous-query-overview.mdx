...
description: "Automatically transform connected-device data into structured datasets that update when data changes or a resource-specific schedule runs."
---

[Continuous Query](/sense/using-continuous-query) automatically transforms raw activity and biometric data from 300+ connected devices into structured datasets that stay up to date. Instead of building complex data pipelines or repeatedly polling APIs, you define your query once and Junction handles the rest—from data ingestion and aggregation to scheduling and delivery.

## How It Works

Continuous Query runs your Junction Sense queries automatically across all users in your team. Junction schedules most queries when new data arrives from cloud providers or [mobile SDKs](/wearables/sdks/health/overview). Virtual resources can use a resource-specific refresh schedule. Junction pushes any result changes to your configured destinations.
...
<Columns cols={2}>
  ...
  <Column>
    <Card title="Intelligent Scheduling" icon="timer">
      Junction responds to new data points or uses a resource-specific refresh schedule, depending on the query
    </Card>
...
<Accordion title="Device Data Reliability">
  Monitor whether expected data has been present without building a separate completeness pipeline.

  **Use case highlights:**

  - Compare presence coverage across providers, source types, and resources
  - Identify stale, sparse, or degraded data feeds
  - Track connection status separately from observed data presence
  - [View Device Reliability →](/sense/query-dsl/device-reliability)
</Accordion>
...
Continuous Query works with the following health data resources and virtual tables:
...
- **Device Reliability** - Daily presence, gap, recency, reliability status, and connection status by provider, source type, and resource
...
[View source prioritization reference →](/sense/data-prioritization)

### Filtering & Refinement

Use [WHERE](/sense/query-dsl/where-clause) clauses to focus on specific data subsets:
...
Use the [Junction Dashboard Query](/sense/managing-queries) Editor to preview resources supported by Query API. For a Continuous Query-only resource such as Device Reliability, the [Create Continuous Query endpoint](/api-reference/sense/continuous-query/create) validates the query when you submit it.
...
Most resources use the same Query DSL with both products, making it easy to prototype with [Query API](/sense/using-query-api) and deploy to production with [Continuous Query](/sense/using-continuous-query). _Some virtual resources, including [Device Reliability](/sense/query-dsl/device-reliability), are only available through Continuous Query and cannot be accessed with Query API._ 
