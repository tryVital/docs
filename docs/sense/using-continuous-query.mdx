...
<Note>
[Device Reliability](/sense/query-dsl/device-reliability) is a Continuous Query-only resource with no datetime index. An ungrouped query must select its provider, source type, and resource identity. A grouped query must use a non-empty subset of those identity columns.
</Note>
