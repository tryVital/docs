...
- `arg: null` → `$COLUMN` (e.g., `menstrual_flow`).
- Struct field extract → `$COLUMN.$FIELD` (e.g., `basal_body_temperature.value`).
- `{ "element": true }` → `$COLUMN.element` (e.g., `tags.element`).

When the subquery is wrapped in an outer aggregate, the outer aggregate function prefix is added following the standard [SELECT clause](/sense/query-dsl/select-clause#aggregate-a-table-column) rules.
