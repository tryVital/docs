...
<Accordion title="Output column name" icon="line-columns" iconType="duotone" defaultOpen>
* the list column name (e.g., `menstrual_flow`) when the subquery aggregates rows directly; or
* `$COLUMN.$FIELD` when a struct field is extracted (e.g., `basal_body_temperature.value`); or
* `$COLUMN.element` when the scalar-list element itself is aggregated.
</Accordion>
