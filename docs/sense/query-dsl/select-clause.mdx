---
title: SELECT clause
description: "Use the SELECT clause in Junction Sense queries to choose table columns, apply aggregate functions, and retrieve group key columns."
icon: hand-pointer
---
...
<Accordion title="Output column name" icon="line-columns" iconType="duotone" defaultOpen>
* `$COLUMN.$FUNC` when the subquery aggregates rows directly (e.g., `menstrual_flow.count`); or
* `$COLUMN.$FIELD.$FUNC` when a struct field is extracted (e.g., `basal_body_temperature.value.mean`); or
* `$COLUMN.element.$FUNC` when the scalar-list element itself is aggregated (e.g., `tags.element.sum`).
</Accordion>
