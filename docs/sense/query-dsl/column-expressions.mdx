...
description: "Reference for table column expressions in Junction Sense queries to select fields from activity, body, sleep, and timeseries resources."
icon: table-columns
---
...
### Continuous Query-only virtual tables

| Table | Python DSL | JSON DSL |
| --- | --- | --- |
| [Device Reliability](/sense/query-dsl/device-reliability)<br/>`reliability` | `Reliability.col("$1")` | `{ "reliability": "$1" }` |

<Note>
Device Reliability Continuous Queries do not support Index Column expressions. Select their provider, source type, and resource identity with `Reliability.col(...)` rather than a Source Column expression.
</Note>
...
| ------------------------------------- | ------------------------------------ | -----------------------------------|----------------------------- |
| Provider                              | `Source.col("source_provider")`      | `{ "source": "source_provider" }`  | Summary and timeseries tables except Readiness Scores and Device Reliability |
| Source Type                           | `Source.col("source_type")`          | `{ "source": "source_type" }`      | Summary and timeseries tables except Readiness Scores and Device Reliability |
| Source App ID                         | `Source.col("source_app_id")`        | `{ "source": "source_app_id" }`    | Summary tables except Readiness Scores and Device Reliability |
...
The Junction resources listed below have a primary datetime index. Your Query Instruction can reference this datetime
