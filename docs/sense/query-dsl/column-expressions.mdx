...
<Note>
Device Reliability has no Index Column expression. Select its provider, source type, and resource identity with `Reliability.col(...)` rather than a Source Column expression.
</Note>
