...
| Sleep snapshot scores<br/>`sleep_snapshot` | `SleepSnapshot.col("$1")` | `{ "sleep_snapshot": "$1" }`
...
| Sleep snapshot scores<br/>`sleep_snapshot` | Calendar date       | `SleepSnapshot.index()` | `{ "index": "sleep_snapshot" }`
