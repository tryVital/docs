...
This query returns one row per provider for each user. `group_key.0` identifies the provider; `mean.presence_coverage` is the unweighted mean across its source-type and resource rows; and `max.presence_recency_day` is the largest non-null recency among them, or `null` if they are all `null`.
