...
Creating a Device Reliability Continuous Query queues its initial snapshot. Junction then refreshes it daily. Each refresh updates the rows identified by provider, source type, and resource; it does not append a history of earlier windows.
...
| `presence_coverage` | Number | No | Present-date ratio: `presence_day_count / 30`. |
...
Device Reliability uses identity columns instead of a datetime index:
