...
description: "Use the GROUP BY clause in Junction Sense queries for date truncation, date part extraction, table column, and source column grouping."
icon: objects-column
iconType: duotone
---
...
* a [Date Truncate expression](#group-by-a-truncated-datetime)
    ...
    * A [Continuous Query](/sense/using-continuous-query) must usually have exactly one [Date Truncate expression](#group-by-a-truncated-datetime).
    * [Device Reliability](/sense/query-dsl/device-reliability) is a Continuous Query-only exception. It does not support Date Truncate grouping and instead groups by a non-empty subset of its provider, source type, and resource identity columns.
