...
* a [Date Truncate expression](#group-by-a-truncated-datetime)
    ...
    * [Device Reliability](/sense/query-dsl/device-reliability) is a Continuous Query-only exception. It has no datetime index and groups by a non-empty subset of its provider, source type, and resource identity columns.
