...
Create a [Vital Sign-In Token](/wearables/sdks/authentication#junction-sign-in-token) for your mobile apps to
...
const data = await client.user.getUserSignInToken(
    '<user_id>'
);
```
...
data = client.user.get_user_sign_in_token(
    user_id="<user_id>"
)
```
