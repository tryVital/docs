---
openapi: get /v1/org/{org_id}/ehr_integration/member
title: "List Integration-Managed Members"
description: "List integration-managed members for an organization."
---

import OrgManagementAPI from '/snippets/org-management-api.mdx';

<OrgManagementAPI />


<RequestExample>
```bash cURL
curl --request GET \
  --url https://api.management.junction.com/v1/org/{org_id}/ehr_integration/member \
  --header 'X-Management-Key: <management-key>'
```
</RequestExample>
