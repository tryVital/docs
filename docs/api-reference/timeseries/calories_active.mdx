...
data = client.vitals.calories_active(user_id="<user_id>", start_date="2021-10-01", end_date="2021-10-02")
```

```javascript Node
import { VitalClient, VitalEnvironment } from '@tryvital/vital-node';
import { VitalsCaloriesActiveRequest } from '@tryvital/vital-node/api/resources/vitals';
...
const request: VitalsCaloriesActiveRequest = {
    startDate: "2022-05-01",
    endDate: "2022-06-01"
}

const data = await client.vitals.caloriesActive(
    '<my_user_id>',
    request
);
```
...
VitalsCaloriesActiveRequest request = VitalsCaloriesActiveRequest.builder()
        .startDate("2022-05-01")
        .endDate("2022-06-01")
        .build();

var data = vital.vitals().caloriesActive("<user_id>", request);
```
...
response, err := client.Vitals.CaloriesActive(context.TODO(), "*", request)
if err != nil {
  return err
}
