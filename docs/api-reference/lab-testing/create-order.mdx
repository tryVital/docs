...
#### At-Home Phlebotomy Order Auto-Approval

<FeatureBeta />

This beta applies only to auto-approval of at-home phlebotomy tests. An inline or derived at-home phlebotomy lab test from `order_set`, including one with `add_on` markers, is subject to the same auto-approval validation as `POST /v3/lab_tests`. For eligible teams and labs, all markers must be *à la carte* enabled unless the team uses a delegated flow, have available collection instructions, and have an estimated draw of fewer than 15 tubes. If validation fails, the request returns HTTP 400 and no derived lab test or order is created.
