...
<Note>
`booking_key` values do not place a hold on Quest PSC capacity. Availability can change between the availability and booking calls, including because the same slot was booked outside Junction. Use `X-Idempotency-Key` only to safely retry the same booking request when that feature is enabled for your team; it does not reserve a slot.
</Note>
