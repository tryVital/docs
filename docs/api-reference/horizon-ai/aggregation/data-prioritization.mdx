...
Within each group outlined by your [Group By clause](/api-reference/horizon-ai/query-dsl/group-by-clause), the query executor would:

* Sub-group the data further by their provider and their [Source Type](/wearables/providers/data-attributions);
* Keep only data from the sub-group that has the highest source type priority, or the highest provider priority if multiple sub-group
