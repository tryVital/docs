...
import { VitalsGlucoseRequest } from '@tryvital/vital-node/api/resources/vitals';
...
const request: VitalsGlucoseRequest = {
...
VitalsGlucoseRequest request = VitalsGlucoseRequest.builder()
        .startDate("2022-05-01")
        .endDate("2022-06-01")
        .build();

var data = vital.vitals().glucose("<user_id>", request);
```
...
response, err := client.Vitals.Glucose(context.TODO(), "*", request)
if err != nil {
  return err
}
