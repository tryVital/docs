...
import { VitalsHeartrateRequest } from '@tryvital/vital-node/api/resources/vitals';
...
const request: VitalsHeartrateRequest = {
...
VitalsHeartrateRequest request = VitalsHeartrateRequest.builder()
        .startDate("2022-05-01")
        .endDate("2022-06-01")
        .build();

var data = vital.vitals().heartrate("<user_id>", request);
```
...
response, err := client.Vitals.Heartrate(context.TODO(), "*", request)
if err != nil {
  return err
}
