...
import { VitalsHrvRequest } from '@tryvital/vital-node/api/resources/vitals';
...
const request: VitalsHrvRequest = {
    startDate: "2022-05-01",
    endDate: "2022-06-01"
}

const data = await client.vitals.hrv(
    '<my_user_id>',
    request
);
```
...
VitalsHrvRequest request = VitalsHrvRequest.builder()
        .startDate("2022-05-01")
        .endDate("2022-06-01")
        .build();

var data = vital.vitals().hrv("<user_id>", request);
```
...
response, err := client.Vitals.Hrv(context.TODO(), "*", request)
if err != nil {
  return err
}
